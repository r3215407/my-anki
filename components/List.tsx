      ...
      <div className="flex gap-3 px-5 py-4 overflow-x-auto no-scrollbar snap-x">
        {(['all', Status.NEW, Status.REVIEW, Status.MASTERED] as const).map(f => (
          <button
            ...
            className={`snap-start flex-shrink-0 h-8 px-5 rounded-full font-bold text-sm shadow-bubbly border-2 active:scale-95 transition-all whitespace-nowrap ${filter === f ? 'bg-primary border-primary text-[#4d3b0b]' : 'bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 text-gray-500'}`}
          >
            {f === 'all' ? '全部' : f}
          </button>
        ))}
